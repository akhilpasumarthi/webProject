...
<form action="prob.php?no=<?php echo $num; ?>&parent=<?php echo $na; ?>" method="POST">
<?php
for ($i=0; $i<$num; $i++)
{   echo "Column ".($i+1);
	?>
	<input type="text" name="anything[]" value=""><br><br>
	<?php
}
?>
<input type="submit" name="submi" value="Add column">
</form>
<?php
...
<form action="subheading.php?name=<?php echo $na; ?>" method="POST">
                    <label>How Many Sub Heading Do You Want To Add:</label><br>
                    <input type="text" name="column" value="">
                    <input type="submit" name="submit" value="Add column" target="_blank">
                   </form>
<?php	
}	
?>
